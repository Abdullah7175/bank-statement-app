...
def parse_header(full_text: str) -> Dict[str, Any]:
    ...
    # Look for patterns like "0 Closing Balance 0 Opening Balance"
    balance_pattern_en = re.search(
        r"(\d+(?:\.\d{2})?)\s*Closing Balance\s*(\d+(?:\.\d{2})?)\s*Opening Balance",
        full_text,
        re.I | re.DOTALL
    )
    ...
    if not hdr.get("opening_balance") and not hdr.get("closing_balance"):
        balance_pattern_ar = re.search(
            r"(\d+(?:\.\d{2})?)\s*ﻲﺋﺎﻬﻨﻟﺍ ﺪﻴﺻﺮﻟﺍ\s*(\d+(?:\.\d{2})?)\s*ﻲﺋﺍﺪﺘﺑﻻﺍ ﺪﻴﺻﺮﻟﺍ",
            full_text,
            re.I | re.DOTALL
        )
