...
def parse_header(full_text: str) -> Dict[str, Any]:
    ...
    # Look for the summary section at the very end of the document
    # The format shows: Closing Balance (SAR) Opening Balance (SAR) followed by Arabic labels
    # Pattern: "21825.64 (SAR) 6489.46 (SAR)" followed by Arabic labels
    # Based on debug output, the exact pattern is in the last 500 characters
    
    # First try the exact pattern in the last 500 characters
    balance_match = re.search(
        r"([\d,]+\.\d{2})\s*\(SAR\)\s*([\d,]+\.\d{2})\s*\(SAR\)",
        full_text[-500:],  # Look in the last 500 characters where the exact pattern is
        re.I | re.DOTALL
    )
    if balance_match:
        # First number is closing balance, second is opening balance
        closing_balance = float(clean(balance_match.group(1)).replace(",", ""))
        opening_balance = float(clean(balance_match.group(2)).replace(",", ""))
        hdr["closing_balance"] = closing_balance
        hdr["opening_balance"] = opening_balance
    else:
        # Fallback: try the pattern with Arabic labels in the last 1000 characters
        balance_match = re.search(
            r"([\d,]+\.\d{2})\s*\(SAR\)\s*([\d,]+\.\d{2})\s*\(SAR\)\s*.*?(?:Closing Balance|ﻝﺎﻔﻗﻹﺍ ﺪﻴﺻﺭ).*?(?:Opening Balance|ﻲﺣﺎﺘﺘﻓﻹﺍ ﺪﻴﺻﺮﻟﺍ)",
            full_text[-1000:],  # Look in the last 1000 characters
            re.I | re.DOTALL
        )
        if balance_match:
            closing_balance = float(clean(balance_match.group(1)).replace(",", ""))
